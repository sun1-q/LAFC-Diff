...
class LesionMaskEncoder(nn.Module):
    """【新增】轻量级病灶掩码编码器"""
    ...
    def forward(self, mask, target_size):
        ...
        # 动态自适应池化，保证绝对对齐 U-Net 中层的尺寸
        feat = F.adaptive_avg_pool2d(feat, target_size)
        return feat

class LesionCrossAttentionBlock(nn.Module):
    """【新增】病灶先验跨注意力融合块 (带 Zero-Conv 初始化)"""
    def __init__(self, unet_dim, cond_dim, num_heads=8):
        super().__init__()
        self.norm1 = nn.LayerNorm(unet_dim)
        self.norm2 = nn.LayerNorm(cond_dim)
        self.k_proj = nn.Linear(cond_dim, unet_dim)
        self.v_proj = nn.Linear(cond_dim, unet_dim)
        self.spatial_bias = nn.Conv2d(cond_dim, cond_dim, kernel_size=3, padding=1, groups=cond_dim)
        self.cross_attn = nn.MultiheadAttention(embed_dim=unet_dim, num_heads=num_heads, batch_first=True)
        self.ffn = nn.Sequential(
            nn.Linear(unet_dim, unet_dim * 4),
            nn.GELU(),
            nn.Linear(unet_dim * 4, unet_dim)
        )
        # 【核心】Zero-Convolution 权重，防止初始训练崩盘
        self.alpha = nn.Parameter(torch.zeros(1)) 

    def forward(self, x, condition):
        B, C_u, H, W = x.shape
        condition = condition + self.spatial_bias(condition) # 提取空间位置信息
        x_flat = x.flatten(2).transpose(1, 2)
        cond_flat = condition.flatten(2).transpose(1, 2)
        
        x_norm = self.norm1(x_flat)
        cond_norm = self.norm2(cond_flat)
        
        K = self.k_proj(cond_norm)
        V = self.v_proj(cond_norm)
        attn_out, _ = self.cross_attn(query=x_norm, key=K, value=V)  
        
        # 零初始化残差连接
        x_flat = x_flat + self.alpha * attn_out
        x_flat = x_flat + self.ffn(self.norm1(x_flat))
        return x_flat.transpose(1, 2).reshape(B, C_u, H, W)
...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    # ====== 【新增代码：初始化病灶注入模块】 ======
    # 我们根据当前的 in_ch 动态设定通道数 (通常是 256 或 512)
    self.mask_encoder = LesionMaskEncoder(in_channels=1, out_channels=in_ch)
    self.lesion_cross_attn = LesionCrossAttentionBlock(unet_dim=in_ch, cond_dim=in_ch)
    # ============================================
  ...
  def forward(self, x, time_cond, z, lesion_mask=None):
    # --- U-Net 的主前向传播 ---
  ...
  # ====== 【新增代码：在此处注入病灶特征】 ======
    if lesion_mask is not None and hasattr(self, 'mask_encoder'):
        # 获取当前中间层的长宽 (比如 16x16)
        target_H, target_W = h.shape[2], h.shape[3]
        # 用轻量网络把高分辨率 Mask 压成中层特征
        lesion_feat = self.mask_encoder(lesion_mask, target_size=(target_H, target_W))
        # 过 Cross-Attention 融入 U-Net
        h = self.lesion_cross_attn(h, lesion_feat)
    # ============================================
